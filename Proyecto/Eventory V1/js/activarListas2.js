...
function ActivarLista(){
  ...
  var elchkActivar = document.querySelector('#chkActivar');
  bchkActivar = elchkActivar.checked;

  for (var i = 0; i < idListaEventoLength; i++) {
    if (nIdListaEvento == aIdListaEvento[i]) {
      aEstadoListaEventos[i] = bchkActivar;
    }
...
function CargarInformacion () {
  ...
  for (var i = 0; i < idListaEventoLength; i++) {
    if (aIdListaEvento[i] == nIdListaEvento) {
      ...
      chkActivar.checked    = aEstadoListaEventos[i];
